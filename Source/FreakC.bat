:: Copyrights 2020-2021 Nguyen Phu Minh
...
for /f "delims=" %%n in ('find /c /v "" %output%.fclang') do set "flen=%%n"
set "flen=!flen:*: =!"
<"%output%.fclang" (
	for /l %%c in (1 1 !flen!) do (
		:: deniedToken decides if the code is just straight out stored in a file, or skipped and process in another process.
		set deniedToken=false

		:: printString is a line of code.
		set /p printString=

		for %%a in (!printString!) do (
			set ch=%%a
			if "!ch:~-4!" == "[..]" (
				set ch=!ch:[..]=!
			...
			if "!ch:~-2!" == "{}" (
				...
				for %%i in (!ch!) do (
					set macro=!%%i!
					for %%j in (!macro!) do set printString=!printString:%%a=%%j!
				)
			)
			if "%%a" == "c[]" (
				set fccomment=true
...
:fcversion
echo FreakC DevKit Version 1.0 BETA
exit /b

:: Just a simple function/subroutine for calculating length of a string.
:get_len
set len=0
set str=%~2
:loop
